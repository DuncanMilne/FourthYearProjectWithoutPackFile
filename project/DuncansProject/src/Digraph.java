...
public class Digraph {
 ...
 public void add(Student student) {
  if (preferThisStudentsProject.containsKey(student)) return;
 ...
 public Map<Student,Integer> inDegree () {
     ...
     for (Student s: preferThisStudentsProject.keySet()) inDegreeMap.put(s, 0);       // All in-degrees are 0 initially
 ...
 public List<Student> topSort () {
     ...
     Stack<Student> zeroVerts = new Stack<Student>();        // Stack as good as any here
     for (Student s: degree.keySet()) {
         if (degree.get(s) == 0) zeroVerts.push(s);
     }
     ...
     while (!zeroVerts.isEmpty()) {
         ...
         for (Student studentPrefersThisProject: preferThisStudentsProject.get(s)) {
             ...
             if (degree.get(studentPrefersThisProject) == 0) zeroVerts.push(studentPrefersThisProject);
         }
     }
